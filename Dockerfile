...
RUN pip install --no-cache-dir -U setuptools wheel pip>=20.0.0 \
    ...
    && mv /opt/venv/bin/OctoBot_Market_Making /opt/venv/bin/OctoBot
...
COPY octobot_market_making/config/default_config.json /octobot/octobot/config/default_config.json
